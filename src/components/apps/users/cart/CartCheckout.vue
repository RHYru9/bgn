...
import { useTransactionStore } from '@/stores/apps/transaksiCheckout';
...
const processCheckout = async () => {
  try {
    ...
    thankyou.value = true;
    store.clearCart(); // Clear cart after successful transaction
  } catch (error) {
          ...
          <v-row>
            ...
            <v-col lg="3" cols="12" md="4">
              ...
              <v-dialog v-model="thankyou" fullscreen>
                <Thankyou :transaction="transactionStore.transaction" />
              </v-dialog>
            </v-col>
          </v-row>
        </v-window-item>
